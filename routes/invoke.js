...
var myinvoke = async function(){
    ...
    if(arguments.length == 2){
        try {
            ...
            var args = [rows[1].Number.toString(), status , 'DNA_'+rows[1].id, rows[1].Member_name, rows[1].RRN_hash, rows[0].id, rows[0].Member_name, rows[0].RRN_hash, rows[0].Product_name, rows[0].Product_price.toString(), username];
        ...
        try{
            ...
            var args = [rows[1].Number.toString(), details , 'DNA_'+rows[1].id, rows[1].Member_name, rows[1].RRN_hash, rows[0].id, rows[0].Member_name, rows[0].RRN_hash, rows[0].Product_name, rows[0].Product_price.toString(), username];
            await invoke.invokeChaincode(peer, channelName, chaincodeName, fcn, args, username, orgname);
        } catch (err){
            console.log('invoke error :' + err);
        }
